...
**Pure-Rust audio pipeline.** symphonia decodes (mp3, wav, flac) with magic-byte format detection — the caller never specifies the format. In-house downmix collapses multi-channel to mono. rubato resamples to Whisper's 16 kHz. No FFmpeg. No system audio libs. The pipeline is part of the build artifact, so the checked-in fixtures round-trip end-to-end with no external tooling.
...
v1.0.0 ships **macOS arm64** (`aarch64-apple-darwin`, Apple Accelerate + ruy). **Linux x86_64** (`x86_64-unknown-linux-gnu`, oneMKL + DNNL + OpenMP) is wired in `Cargo.toml`, `package.json`, and the TS dispatcher — the binary is committed by the [Linux follow-up build](docs/backlog.kanban.md).

Windows, GPU, Linux-arm64, and macOS-x64 are deferred and tracked in the same backlog.
...
The complete release process — version bump, rebuild on each host, packaging-allowlist verification, publish — is recorded in [`docs/archive/CONCEPT_v1_buildout.md`](docs/archive/CONCEPT_v1_buildout.md) (Release Runbook).
...
**Audio pipeline (`src/decode.rs`):** Caller bytes → symphonia (decode + magic-byte format detection — caller never specifies format) → in-house downmix (multi-channel → mono via float averaging) → rubato (sinc resampler from native rate to 16 kHz) → `Vec<f32>` at 16 kHz mono in `[-1, 1]`. If the source is already 16 kHz mono, downmix and resample are no-ops. Corrupt input surfaces as `CadmusError::Decode`; pathological sample rates as `CadmusError::Resample`. Three checked-in fixtures (`fixtures/eins-zwei-drei.{mp3,wav,flac}`) exercise three sample rates (22 050, 44 100, 48 000 Hz) so resample is hit on every test run.
...
v1.0.0 ships the macOS arm64 `.node` only. The Linux x86_64 `.node` is committed by the deferred follow-up plan tracked in `docs/backlog.kanban.md`. The Linux plumbing in `Cargo.toml` (per-target ct2rs features), `package.json.files`, and `index.ts` (platform dispatch) is wired so the follow-up only has to run, not author. Other platforms (Windows, Linux-arm64, macOS-x64, GPU variants) are out of scope for v1.

No CI, no GitHub Actions in v1. Verification is local on each build host before each release. The Release Runbook in `docs/archive/CONCEPT_v1_buildout.md` codifies the manual sequence. Migration to CI is a clean addition tracked in `docs/backlog.kanban.md` — none of the v1 build decisions block it.
