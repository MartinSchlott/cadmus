...
- **D4.** Memory model: `CadmusModel` holds the inner `ct2rs::Whisper` as `Arc<Whisper>` plus an atomic "freed" sentinel. No `Mutex` on the inference path: ct2rs explicitly declares the underlying FFI struct `Send + Sync` (`ct2rs/src/sys/whisper.rs:524–525`: `unsafe impl Send for ffi::Whisper`, `unsafe impl Sync for ffi::Whisper`), so concurrent `generate` calls are safe at the type level. `free()` is non-blocking and does not abort in-flight transcriptions — reference-counted deferred release. Value-over-abort. *Implemented in PLAN_inference_core (now archived) as `InferenceHandle { inner: Mutex<Option<Arc<Whisper>>>, freed: AtomicBool }`. The `Mutex` exists solely to make `free()` swap the owning `Arc` out atomically; its critical section spans only the `freed`-check + `Arc::clone` (or `take()` in `free()`), never the call to `Whisper::generate`. The actual inference runs lock-free on the cloned `Arc`, so the "no `Mutex` on the inference path" constraint holds. Three Rust unit tests verify the contract directly: transcribe-after-free → `AlreadyFreed`, free-during-inflight (in-flight call completes normally), concurrent transcribe (two threads succeed in parallel).*
...
| 4 | `PLAN_inference_core.md` | `ct2rs` integration as **crate-internal** machinery only — no public loading API yet. Internal Whisper-handle wrapper implementing D4 directly (`Arc<Whisper>` + atomic freed sentinel; mutex held only around the `Arc` swap, never around `generate` — see D4). Whisper `<\|t\|>` timestamp-token parser → internal `Segment` struct, UTF-8-safe via string-slice accumulation. End-to-end crate-internal test under `#[cfg(test)]` — lives inside `src/` — calls Plan 3's downloader to ensure the model is staged, decodes the fixture via Plan 2's pipeline, runs inference, parses segments, and asserts that all three numbers (eins/1, zwei/2, drei/3) are recognised in either spoken or digit form (Human-amended A3 — tiny normalises spoken numbers to digits on this fixture; the original "contains 'eins'" anchor was falsified during first validation and broadened by Human decision). Plus dedicated Rust unit tests for D4: free-after-free → `AlreadyFreed`; free-during-inflight (two threads — one in `generate`, the other calls `free` mid-flight; in-flight call must complete normally, native instance released only after the last `Arc` clone drops); concurrent `transcribe` (two threads invoking on the same handle in parallel). Adds `InferenceError` with distinct `AlreadyFreed` / `Poisoned` variants (lock-poisoning is surfaced separately so Plan 5 can map one-to-one onto definition §4.3's `CadmusError::Poisoned`). Removes `#![allow(dead_code)]` from `src/decode.rs` and `src/storage.rs` (this plan is the first consumer of both). **No** public API. | `cargo test` green: end-to-end substring-set assertion holds; the three D4 tests pass; the five segment-parser synthetic tests (incl. UTF-8 umlaut and malformed-token cases) pass. Build-verification gate is `cargo build --release --tests` (Human-amended): plain `cargo build --release` cannot be clean while every `pub(crate)` item is consumed only from `#[cfg(test)]` blocks, by design. The crate exports nothing user-callable that loads or transcribes — those land in Plan 5. |
...
│   ├── inference.rs                  # added in Plan 4 (crate-internal Whisper handle + segment parser; D4)
